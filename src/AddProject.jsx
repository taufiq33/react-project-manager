    ...
    <section className="flex flex-col p-4 w-full">
      <form onSubmit={onSave}>
        <div className="flex justify-end p-2 gap-8 mb-2">
          ...
          <button type="submit" className="text-sm px-8 py-2 bg-slate-700 text-white rounded-lg shadow-md hover:bg-slate-500">Save</button>
        </div>
        <div>
          <p>
          ...
          <p>
            ...
            <textarea required ref={descRef} className="border-0 border-b border-slate-400 rounded-md p-1 text-sm w-full bg-gray-200" name="description" id=""></textarea>
          </p>
          <p>
            <label htmlFor="dueDate" className="text-slate-900 tracking-widest font-xl block mt-6 mb-1">DUE DATE</label>
            <input required onChange={handleDueDateChange} ref={dateRef} className="border-0 border-b border-slate-400 rounded-md p-1 text-sm w-full bg-gray-200" type="date" name="dueDate" />
            <span>{dueDate}</span>
          </p>
        </div>
      </form>
